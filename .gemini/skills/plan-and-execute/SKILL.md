...
3. **Read the plan** from `docs/plans/` and dispatch according to topology. All topologies use a **two-stage review** (spec compliance then code quality) -- adapted to the execution unit (task for A/B, agent for C).
   ...
   > ```
   > Use ralph-loop per-task only for iterative/convergence tasks where the agent needs multiple self-correction passes and success criteria are automatable (tests, lint, reviewer checks). Do NOT use it for tasks requiring human judgment, one-shot operations, or tasks with unclear success criteria. For most tasks, the SDD dispatch below is sufficient and handles review automatically.
...
10. **RALPH Finalization Loop:**
    ...
    **Skip ralph-loop when:** tasks requiring human judgment (UX, design trade-offs), one-shot operations (migrations, data fixes), or unclear success criteria that can't be expressed as automated checks. In these cases, run the validation suite once manually and present results to the user for review.

    | Signal | Use ralph-loop? | Instead |
    |--------|----------------|---------|
    | All RALPH criteria are automatable (tests, lint, reviewer) | Yes | — |
    | Success depends on subjective quality (UX, naming, architecture) | No | Run checks once, present to user |
    | One-shot operation (migration, seed data, config change) | No | Verify manually, no iteration needed |
    | Criteria require external validation (API contracts, staging) | No | Run checks once, escalate to user |
